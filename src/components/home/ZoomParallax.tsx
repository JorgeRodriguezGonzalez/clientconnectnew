...
const ParallaxVideo = ({ src, objectPosition = 'center' }: { src: string, objectPosition?: string }) => {
    ...
    const handlePlay = () => {
        ...
        if (p !== undefined) {
            p.catch(() => {
                setTimeout(() => { if (video.paused) video.play().catch(() => null); }, 100);
            });
        }
    };
    ...
    useEffect(() => {
        ...
        const obs = new IntersectionObserver((entries) => {
            entries.forEach((e) => { e.isIntersecting ? video.play().catch(() => null) : video.pause(); });
        }, { threshold: 0.3 });
        obs.observe(video);
        return () => { obs.disconnect(); };
    }, [isTablet]);
    return (
        <video ref={videoRef} src={src} onCanPlay={handlePlay} onLoadedData={handlePlay}
            ...
            <div className="flex gap-3 h-full">
                <div className="flex-1 overflow-hidden">
                    <div style={{ animation: 'scrollUp 35s linear infinite' }}>
                        <div className="flex flex-col gap-3">
                            {[...mobileCol1Images, ...mobileCol1Images].map((item, i) => (
                                <a key={i} href={item.href} target="_blank" rel="noopener noreferrer"
                                    className="aspect-[4/5] overflow-hidden rounded-[16px] border border-white/10 bg-[#1a1a1a] block">
                ...
                <div className="flex-1 overflow-hidden">
                    <div style={{ animation: 'scrollDown 38s linear infinite' }}>
                        <div className="flex flex-col gap-3">
                            {[...mobileCol2Images, ...mobileCol2Images].map((item, i) => (
                                <a key={i} href={item.href} target="_blank" rel="noopener noreferrer"
                                    className="aspect-[4/5] overflow-hidden rounded-[16px] border border-white/10 bg-[#1a1a1a] block">
                                    <img src={item.src} alt="" loading="lazy" className="h-full w-full object-cover" />
                                </a>
                                        ...
                                        <motion.div style={{ opacity: contentOpacity, scale: inverseScale, pointerEvents: 'auto' as any }}
                                            ...
                                            <div className="flex gap-3 mt-2">
                                                ...
                                                <motion.a
                                                    ...
                                                    style={{
                                                        fontSize: '15px', height: '48px', padding: '12px 24px',
                                                        borderRadius: '50px',
                                                ...
                                                <motion.a
                                                    ...
                                                    style={{
                                                        ...
                                                        color: `${COLORS.cyan}`, transition: 'all 0.3s ease',
                                                    }}>
                                                    Book a Call
                                                </motion.a>
                                            </div>
                                            <motion.div className="flex flex-col items-center gap-1 mt-4 text-white/40 cursor-pointer"
                                                animate={{ y: [0, 6, 0] }}
        ...
        <section className="w-full bg-[#050505] text-white">
            ...
            <div className="relative flex h-[45vh] md:h-[70vh] flex-col items-center justify-center gap-8 px-4 overflow-hidden">
                    ...
                    <h2 className="section-title text-white max-w-4xl md:whitespace-nowrap">
                        ...
                        <motion.span
                            ...
                            style={{
                                ...
                                backgroundSize: "400% 100%",
                                WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent",
                                backgroundClip: "text", color: "transparent", paddingBottom: "10px"
                            }}>
                            convert &amp; grow.
                        </motion.span>
                    </h2>
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                    ...
                    <span className="font-satoshi uppercase" style={{ fontSize: '10px', fontWeight: 600, letterSpacing: '2px' }}>Explore</span>
                    <ArrowDown className="w-5 h-5 animate-bounce" />
                </motion.div>
            </div>
            {isMobile ? <MobileCarousel /> : <ParallaxContent videos={videos} isMobile={isMobile} />}
