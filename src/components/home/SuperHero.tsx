...
const CampaignProgressWidget = ({ className = "" }: { className?: string }) => {
  ...
  return (
    <div className={`w-full max-w-[280px] bg-[#1a1a1a] rounded-[18px] px-4 py-4 shadow-xl border border-[#333333] ${className}`}>
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-green-500 text-xs font-bold bg-green-500/10 px-2 py-1 rounded-full">Active</span>
      </div>
      
      <div className="flex items-end gap-2 mb-4">
        <h2 className="text-2xl font-black tracking-tight text-white">{roiValue}%</h2>
        <span className="text-green-500 text-sm font-medium mb-1">ROI ▲</span>
      </div>

      <div className="relative mb-4 px-1">
        <div className="h-1.5 bg-[#404040] rounded-full w-full" />
        <motion.div 
          className="absolute top-0 left-1 h-1.5 bg-gradient-to-r from-emerald-400 to-green-600 rounded-full" 
          initial={{ width: "16.67%" }} 
          animate={{ width: step === 3 ? "95%" : step === 2 ? "60%" : "16.67%" }} 
...
const MarketingAppContent = ({ onOptionChange }: { onOptionChange: (id: string) => void }) => {
  const [messages, setMessages] = useState<Array<{id: number, type: 'sent' | 'received', content: React.ReactNode}>>([
    { 
      id: 1, 
      type: 'received', 
      content: "Gday mate. Where do you want us to focus our energy now?" 
    }
  ]);
  ...
  return (
    <div className="flex flex-col h-full bg-white relative font-sans overflow-hidden">
      <div className="h-[44px] flex items-center justify-between px-6 pt-2 z-20 shrink-0 bg-white/90 backdrop-blur-md sticky top-0 border-b border-gray-100">
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 bg-white">
        {messages.map((msg) => (
          ...
          >
            <div 
              className={`max-w-[85%] px-4 py-2.5 text-[15px] leading-snug shadow-sm
                ${msg.type === 'sent' 
      ...
      <AnimatePresence>
        {showOptions && (
          <motion.div 
            ...
            className="p-4 bg-gray-50/90 backdrop-blur-lg border-t border-gray-200 z-10"
          >
            <div className="flex flex-col gap-2">
              <span className="text-xs text-gray-400 font-medium ml-2 mb-1 uppercase tracking-wider">Suggested Replies</span>
              {initialPreferences.map((preference, index) => (
                <motion.button 
                    key={preference.id} 
                    onClick={() => handleSelect(preference)} 
                    whileTap={{ scale: 0.98 }}
                    className="w-full text-left bg-white active:bg-gray-100 border border-gray-200 text-blue-600 font-medium py-3 px-4 rounded-xl shadow-sm hover:shadow transition-all text-[15px]"
                >
                  {preference.label}
                </motion.button>
              ))}
      ...
      <div className="h-[20px] bg-white w-full flex items-center justify-center shrink-0">
         <div className="w-[130px] h-[5px] bg-black/90 rounded-full mb-2" />
      </div>
    </div>
  );
};
...
export const SuperHero = ({
  ...
  return (
    // FIX: Aumentado el padding-top (pt-32) para bajar todo el contenido (Lámpara, Título, etc.)
    <div className="w-full min-h-screen bg-gradient-to-br from-black via-[#050505] to-[#15171c] flex flex-col items-center justify-start pt-32 px-0 overflow-hidden relative">
      ...
      {/* --- FIX: Altura restaurada a h-[850px] para que la sección de animaciones sea grande --- */}
      <div className="w-full relative h-[850px] flex justify-center overflow-hidden z-[10] -mt-24">
        ...
        {/* FIX: justify-end + translate-y-[20%] para que el teléfono esté abajo, lejos de los botones y cortado por el borde inferior */}
        <div className="relative z-[20] flex flex-col items-center justify-end transform translate-y-[20%] scale-[0.8] md:scale-[0.85] lg:scale-[0.9] origin-bottom">
