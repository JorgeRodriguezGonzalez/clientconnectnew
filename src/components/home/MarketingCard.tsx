...
export const MarketingCard = ({
  ...
  return (
    <motion.div 
      ref={ref}
      className="relative flex flex-col items-center gap-1.5 w-[685.2px] h-[310px] rounded-[4px] overflow-hidden p-4"
      style={{
...
function AnimatedIcons() {
  const icons = [
    ...
    {
      icon: <CopilotLogo className="h-6 w-6" />,
      size: "md" as const,
    },
    {
    ...
    <div className="overflow-hidden h-full relative flex items-center justify-center">
      <div className="flex flex-row flex-shrink-0 justify-center items-center gap-2">
        {icons.map((icon, index) => (
          <Container
            key={index}
            className={`${sizeMap[icon.size || "lg"]} circle-${index + 1}`}
          >
            {icon.icon}
          </Container>
        ))}
...
const AnimatedSparkles = () => (
  <div className="h-40 w-px absolute top-1/2 -translate-y-1/2 z-40 bg-gradient-to-b from-transparent via-cyan-500 to-transparent animate-move-horizontal">
