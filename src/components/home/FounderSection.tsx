...
// --- LOGO COMPONENTS PARA SCANNER (CALCO EXACTO) ---
...
const MetaIconOutline = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 287.56 191" className={className}><path fill="currentColor" d="M31.06,126c0,11,2.41,19.41,5.56,24.51A19,19,0,0,0,53.19,160c8.1,0,15.51-2,29.79-21.76,11.44-15.83,24.92-38,34-52l15.36-23.6c10.67-16.39,23-34.61,37.18-47C181.07,5.6,193.54,0,206.09,0c21.07,0,41.14,12.21,56.5,35.11,16.81,25.08,25,56.67,25,89.27,0,19.38-3.82,33.62-10.32,44.87C271,180.13,258.72,191,238.13,191V160c17.63,0,22-16.2,22-34.74,0-26.42-6.16-55.74-19.73-76.69-9.63-14.86-22.11-23.94-35.84-23.94-14.85,0-26.8,11.2-40.23,31.17-7.14,10.61-14.47,23.54-22.7,38.13l-9.06,16c-18.2,32.27-22.81,39.62-31.91,51.75C84.74,183,71.12,191,53.19,191c-21.27,0-34.72-9.21-43-23.09C3.34,156.6,0,141.76,0,124.85Z" /></svg>
);
const GeminiLogo = ({ className }: { className?: string }) => (
  <svg viewBox="0 0 16 16" className={className}><path d="M16 8.016A8.522 8.522 0 008.016 16h-.032A8.521 8.521 0 000 8.016v-.032A8.521 8.521 0 007.984 0h.032A8.522 8.522 0 0016 7.984v.032z" fill="currentColor" /></svg>
);

// --- SCANNER LOGIC (CALCO EXACTO) ---
...
function TechnicalScanner({ isLightMode }: { isLightMode: boolean }) {
  const icons = [
    ...
    { icon: <OpenAILogo className="h-6 w-6" />, size: "lg" as const },
  ...
  return (
    <div className="overflow-hidden h-full relative flex items-center justify-center w-full">
      <div className="flex flex-row flex-shrink-0 justify-center items-center gap-2">
        {icons.map((icon, i) => (
          <LogoContainer key={i} className={`${sizeMap[icon.size]} logo-circle-${i + 1} ${isLightMode ? "bg-white/90 text-black" : "bg-neutral-800/80 text-white"}`}>{icon.icon}</LogoContainer>
        ))}
      </div>
  ...
  return (
    <section ref={containerRef} className={cn("relative w-full transition-colors duration-500 font-sans", isLightMode ? "bg-[#FAFAFA]" : "bg-[#050505]")}>
      ...
      <div className="max-w-[1200px] mx-auto px-6 md:px-12 lg:px-16 relative z-10 pb-20">
        ...
        {/* --- COMPACT INTRO SECTION (MÁS SEPARACIÓN Y DESPLIEGUE) --- */}
        <div className="lg:grid lg:grid-cols-12 gap-12 items-center mb-16 pt-5">
          ...
          <div className="lg:col-span-6 relative h-[380px] lg:pl-20 mt-10 lg:mt-0">
             {[
               { id: 1, title: "Agency #1", text: "$8K Website. Looked amazing. Got zero leads. No SEO, just a pretty brochure no one saw.", y: 0, x: -40, r: -12, z: 10 },
               { id: 2, title: "Agency #2", text: "$12K Google Ads. Got clicks, but the website was so shit no one called. They blamed us.", y: 80, x: 0, r: 0, z: 20 },
               { id: 3, title: "Agency #3", text: "'SEO experts'. Vanity metrics only. Ranking for keywords no one searches. Zero jobs.", y: 160, x: 40, r: 12, z: 30 },
             ].map((card) => (
               <motion.div
                key={card.id}
                ...
                className={cn(
                  "absolute inset-x-0 p-6 border backdrop-blur-md transition-all duration-300 cursor-help shadow-2xl",
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-16 items-start mt-10">
          ...
          <div className="lg:w-[60%] relative">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5 auto-rows-fr">
              ...
              {/* TECHNICAL SEO SCANNER CARD (CLONADO EXACTO) */}
